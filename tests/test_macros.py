...
from lambdora.values import Macro, nil, valueToString
...
def test_macro_expand_simple():
    env = {
        "foo": Macro(
            ["a", "b"], Application(Variable("+"), [Variable("a"), Variable("b")])
        )
    }
...
def test_macro_expand_nested_args():
    env = {"bar": Macro(["x"], Application(Variable("print"), [Variable("x")]))}
